...
export class FfmpegService implements OnApplicationBootstrap {
    ...
    private supportedFormatsCache: Array<{ name: string; description: string; canDemux: boolean; canMux: boolean }> | null = null;
    
    constructor(
        private readonly ffmpegDownloaderService: FfmpegDownloaderService
    ) {}
    ...
    private spawnFfmpeg(args: string[]): ChildProcess {
        const ffmpegPath = this.ffmpegDownloaderService.getFfmpegPath();
        return spawn(ffmpegPath, args);
    }

    private spawnFfprobe(args: string[]): ChildProcess {
        const ffprobePath = this.ffmpegDownloaderService.getFfprobePath();
    ...
    public getVideoMetadata(filePath: string): Promise<any | null> {
        return new Promise((resolve) => {
            const args = [
                "-v", "error",
                "-show_format",
    ...
    public getSupportedFormats(): Promise<Array<{ name: string; description: string; canDemux: boolean; canMux: boolean }>> {
        return new Promise((resolve) => {
            const args = ["-formats"];
    ...
    private getImageFormatName(ext: string): string {
        ...
        const hasExactMatch = this.supportedFormatsCache.some(f => 
            f.name.split(',').map(n => n.trim()).includes(ext)
        );
